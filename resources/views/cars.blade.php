...
<div class="container">
    <div class="row my-5 d-flex justify-content-evenly">
        <form action="{{ route('car.search') }}" method="POST" class="search d-flex col-4 ">

            @csrf
            <div>
                <input type="search" class="form-control border-0" name="car_name" required id="" value="@php if(isset($_POST['car_name'])){echo $_POST['car_name'];}@endphp" placeholder="Search car">
            </div>
        ...
        <form action="{{ route('car.filter') }}" method="POST" class="col-4 d-flex ">
            @csrf
        ...
        <button data-bs-toggle="modal" data-bs-target="#modelId" type="button" class="btn col-2 border" style="background-color: rgb(251, 185, 34);color:white; border-radius: 10px">
            +Car
        </button>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form action="{{ route('car.store') }}" method="POST">
                            ...
                            <button type="submit" class="btn btn-info" id='submit'>Ajouter</button>
    ...
    <div class="row bg-white py-4 px-3 d-flex justify-content-evenly rounded">
        @forelse ($cars as $car)
        <div class="col-lg-3 col-md-6 col-11">
            <div class="card my-lg-2 my-4 position-relative">
                ...
                <span class="tarif position-absolute top-0 end-0 py-2 px-2">{{ $car->tarifLocation }} XOF</span>
                <div class="card-body">
                    <h6 class="card-title mb-3 text-center">{{ $car->marque }} - {{$car->model}} {{$car->yearFabrication}}</h6>
                    <div class="d-flex justify-content-between">
                        @if ($car->status === "Indisponible")
                        <span class="text-danger">{{ $car->status }}</span>
                        @else
                        <span class="text-success">{{ $car->status }}</span>
                        @endif
                        <div>
                            <a href="{{ route('car.to_update', ['immatriculation' => $car->immatriculation]) }}"><svg width="15" height="32" viewBox="0 0 28 32" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <a href="{{ route('car.delete', ['immatriculation'=>$car->immatriculation]) }}"><svg width="15" height="26" viewBox="0 0 25 26" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        @empty
        <div>
            Aucune voiture n'a été ajoutée
        </div>
